...
def update_sheet_cells(client, sheet_name, worksheet_name, updates):
    ...
    [{'range': 'A1', 'value': 'New Value'}, {'range': 'B2:C3', 'values': [[1, 2], [3, 4]]}]
    """
    try:
        sheet = client.open(sheet_name).worksheet(worksheet_name)
        update_requests = []
        ...
        for update in updates:
            # Separate batch updates (ranges with 'values') from single cell updates ('value')
            if 'values' in update:
                update_requests.append({
                 ...
                 # gspread batch_update doesn't handle single 'value' well, use update for these
                 # Collect single updates to perform them individually
                 single_updates.append(update)
            else:
                st.warning(f"Skipping invalid update format: {update}")

        # Perform batch updates first if any
        if update_requests:
             sheet.batch_update(update_requests, value_input_option='USER_ENTERED')

        # Perform single cell updates individually
        for update in single_updates:
            sheet.update(update['range'], update['value'], value_input_option='USER_ENTERED')

        st.success(f"Successfully updated cells in '{worksheet_name}'.")
        return True
    except Exception as e:
        ...
        for emp_id, deduction in employee_updates.items():
            ...
            if not match.empty:
                ...
                try:
                    ...
                    updates.append({'range': cell_ref, 'value': new_balance})
                except KeyError:
    ...
    try:
        ...
        for proj_id, ads_added in project_updates.items():
            ...
            if not match.empty:
                ...
                # Ensure value is suitable for Sheets API (numeric)
                updates.append({"range": cell_ref, "value": float(new_ads_count)})
            else:
...
def update_global_budget(client, sheet_name, total_ads_deducted):
    ...
    try:
        ...
        update = [{'range': 'A2', 'value': float(new_balance)}]
    ...
    try:
        ...
        for index, employee in employees_df.iterrows():
            ...
            if not match.empty:
                ...
                cell_ref = gspread.utils.rowcol_to_a1(row_index, col_index)
                update_requests.append({'range': cell_ref, 'value': float(initial_balance)})
            else:
